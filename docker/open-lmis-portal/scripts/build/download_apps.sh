...
cp /tmp/builds/openlmis-web.war /usr/local/tomcat/webapps/ROOT.war

#download migration jars
mkdir -p /usr/local/tomcat/webapps/db/
cp /tmp/builds/db.jar /usr/local/tomcat/webapps/db/
cp /tmp/builds/migration.jar /usr/local/tomcat/webapps/db/
...
cp -R /tmp/configuration/*.properties /usr/tomcat/extra_properties/
