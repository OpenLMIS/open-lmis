...
//  }
  ...
  public void shouldNotThrowExceptionIfFeatureToggleOnForAllowingPreviousReq() {
    ...
    when(requisitionService.getNormalRnrsByPeriodAndProgram(null, null, reportingProgram.getId(), reportingFacility.getId())).thenReturn(null);
  ...
  public void shouldThrowExceptionIfFeatureToggleOnAndUploadingRnrWithSamePeriod() {
    ...
    when(requisitionService.getPeriodForInitiating(reportingFacility, reportingProgram)).thenReturn(previousPeriod);
    when(requisitionService.getNormalRnrsByPeriodAndProgram(beginDate, endDate, reportingProgram.getId(), reportingFacility.getId())).thenReturn(asList(new Rnr()));
