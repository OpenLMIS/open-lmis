...
function BudgetFileTemplateController($scope, budgetFileTemplate, BudgetFileTemplate, dateFormats, $location, messageService) {
  ...
  function isDuplicatePosition() {
    var positionList = _.pluck(_.where($scope.budgetFileTemplate.columns, {"include": true}), "position");
  ...
  function isInvalidPosition() {
    ...
    angular.forEach($scope.budgetFileTemplate.columns, function (column) {
...
BudgetFileTemplateController.resolve = {
  budgetFileTemplate: function ($q, $timeout, BudgetFileTemplate) {
    var deferred = $q.defer();
    $timeout(function () {
      BudgetFileTemplate.get({}, function (data) {
        deferred.resolve(data.budget_template);
      }, {});
    }, 100);
