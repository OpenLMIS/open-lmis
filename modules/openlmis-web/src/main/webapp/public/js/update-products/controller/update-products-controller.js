...
function UpdateProductsController($scope, messageService, loginConfig, $window) {
    ...
    $scope.$on('$viewContentLoaded', function () {
        ...
        $('#updateProductsForm').ajaxForm(options);
    });
    ...
    $scope.validate = function (formData) {
        $scope.$apply(function () {
            ...
            $scope.successMsg = $scope.errorMsg = "";
            if (setErrorMessageIfEmpty(formData[0].value, 'csvFile', 'upload.select.file')) {
    ...
    function setErrorMessageIfEmpty(value, fieldName, messageKey) {
        if (utils.isEmpty(value)) {
            $scope.updateProductsForm[fieldName].errorMessage = messageService.get(messageKey);
            return true;
        } else {
            $scope.updateProductsForm[fieldName].errorMessage = "";
            return false;
        }
    }
