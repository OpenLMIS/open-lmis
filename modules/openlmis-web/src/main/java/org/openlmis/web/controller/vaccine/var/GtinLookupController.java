...
import org.openlmis.vaccine.service.var.GtinLookupsService;
...
@Controller
@RequestMapping(value = "/vaccine/gitn_lookup")
public class GtinLookupController  extends BaseController {

    @Autowired
    private GtinLookupsService service;

    @RequestMapping(value="get/{id}")
    public ResponseEntity<OpenLmisResponse> get(@PathVariable Long id) {
        return OpenLmisResponse.response("gitn_lookup", service.getById(id));
    }
    ...
    public ResponseEntity<OpenLmisResponse> getAll() {
        return OpenLmisResponse.response("gitn_lookup", service.getAll());
    }

    @RequestMapping(value="save", headers = ACCEPT_JSON, method = RequestMethod.POST)
    @Transactional
    public ResponseEntity<OpenLmisResponse> save(@RequestBody GtinLookups gitn_lookup) {
        try {
            service.save(gitn_lookup);
        } catch (DataException e) {
            return OpenLmisResponse.error(e, BAD_REQUEST);
        }
        return OpenLmisResponse.response("gitn_lookup", service.getById(gitn_lookup.getId()));
    }
}
