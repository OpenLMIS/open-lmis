DROP VIEW dw_product_fill_rate_vw;
...
CREATE OR REPLACE VIEW dw_product_fill_rate_vw AS 
		...
		CASE  WHEN COALESCE(sum(COALESCE(dw_orders.quantityapproved, 0)::numeric), 0::numeric) = 0::numeric THEN 0::numeric
             ELSE (sum(
                CASE
                    WHEN COALESCE(dw_orders.quantityreceived, 0) = 0 THEN dw_orders.quantityshipped::numeric
                    ELSE COALESCE(dw_orders.quantityreceived, 0)::numeric
                END)) / (sum(COALESCE(dw_orders.quantityapproved, 0)::numeric)) * 100::numeric
