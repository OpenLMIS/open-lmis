-- Trigger: tg_requisitions_insert_update_delete on requisitions
DROP TRIGGER IF EXISTS tg_requisitions_insert_update_delete ON requisitions;

 -- Function: fn_populate_dw_rnr()

DROP FUNCTION IF EXISTS fn_populate_dw_rnr();

CREATE OR REPLACE FUNCTION fn_populate_dw_rnr()
  RETURNS trigger AS
$BODY$
DECLARE
rec RECORD ;
...
IF (TG_OP = 'INSERT') THEN
 DELETE FROM dw_orders where rnrid = NEW.id; -- OLD.id is null
ELSEIF (TG_OP = 'UPDATE') THEN
 DELETE FROM dw_orders where rnrid = NEW.id; -- OLD.id = NEW.id
ELSEIF (TG_OP = 'DELETE') THEN
 DELETE FROM dw_orders where rnrid = OLD.id; -- OLD.id = NEW.id
...
CREATE TRIGGER tg_requisitions_insert_update_delete
  ...
  ON requisitions
  FOR EACH ROW
  EXECUTE PROCEDURE fn_populate_dw_rnr();
