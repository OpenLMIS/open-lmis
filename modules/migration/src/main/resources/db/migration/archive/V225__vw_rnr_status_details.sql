...
CREATE OR REPLACE VIEW vw_rnr_status_details AS
 SELECT p.name AS programname, r.programid, r.periodid, ps.name AS periodname, rg.id AS requisitiongroupid, r.createddate, f.code AS facilitycode, f.name AS facilityname, rg.name AS requisitiongroup, f.id AS facilityid, r.id AS rnrid, r.status
   FROM facilities f
