...
CREATE OR REPLACE VIEW vw_order_fill_rate AS
 ...
 SELECT r.programid, programs.name AS program, li.productcategory AS category,
    ...
    requisition_group_members.id AS rgroupid, rg.name AS rgroup,
    ...
    li.quantityapproved AS approved,
        CASE
            WHEN sum(li.quantityapproved) > 0 THEN round(sum(li.quantityreceived::numeric) / sum(li.quantityapproved)::numeric * 100::numeric, 0)
            ELSE 0::numeric
  ...
  GROUP BY li.product, li.productcategory, f.name, ft.name, li.productcode, li.quantityapproved, li.quantityreceived, gz.name, r.programid, programs.name, prc.id, pp.id, ps.id, ft.id, requisition_group_members.id, rg.name, f.id, pr.id, s.name, gz.id;
