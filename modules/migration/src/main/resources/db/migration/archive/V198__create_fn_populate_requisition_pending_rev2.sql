...
delete from alert_summary where supervisorynodeid = rec.supervisorynodeid and programid = rec.programid and alerttypeid = 'REQUISITION_PENDING';
...
INSERT INTO alert_summary(
    ...
    VALUES (rec.rec_count, ' Requisitions Pending', rec.supervisorynodeid, 'REQUISITION_PENDING', rec.programid);
...
LOOP --fetch the table row inside the loop
  ...
  select id into v_summaryid from alert_summary where programid = rec2.programid and supervisorynodeid = rec2.supervisorynodeid and alerttypeid = 'REQUISITION_PENDING';
