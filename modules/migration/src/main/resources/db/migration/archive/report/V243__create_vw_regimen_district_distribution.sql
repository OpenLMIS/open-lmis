...
CREATE OR REPLACE VIEW vw_regimen_district_distribution AS
 SELECT DISTINCT r.programid, ps.id AS scheduleid, pp.id AS periodid, rg.id AS rgroupid, regimens.categoryid, 
 regimens.id AS regimenid, regimens.name AS regimen, gz.name AS district, li.patientsontreatment, 
 li.patientstoinitiatetreatment, li.patientsstoppedtreatment, r.facilityid, r.status, f.name AS facilityname, f.typeid AS facilitytypeid
