...
SELECT dw_orders.programid,dw_orders.periodid,dw_orders.requisitionGroupId,
   ...
   INNER JOIN requisition_group_program_schedules rps on rps.requisitionGroupId = dw_orders.requisitionGroupId and rps.programId = dw_orders.programid
  WHERE dw_orders.status::text = ANY (ARRAY['APPROVED'::character varying::text, 'RELEASED'::character varying::text])
  GROUP BY dw_orders.programid,dw_orders.periodid,dw_orders.requisitionGroupId,
    dw_orders.productid, products.primaryname,dw_orders.facilityid;
