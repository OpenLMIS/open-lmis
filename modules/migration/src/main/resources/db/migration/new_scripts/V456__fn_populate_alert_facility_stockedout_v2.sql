...
-- select * from dw_orders where modifieddate::date = '2015-04-10'
...
-- delete summary data updated on/after last run data warehouse update 
delete from alert_summary where alerttypeid = 'FACILITY_STOCKED_OUT_OF_TRACER_PRODUCT'
and  modifieddate >= v_last_run_date;

delete from alert_facility_stockedout where alertsummaryid not in (select alertsummaryid from alert_summary);
...
--DELETE FROM alert_facility_stockedout where modifieddate >= v_last_run_date;
