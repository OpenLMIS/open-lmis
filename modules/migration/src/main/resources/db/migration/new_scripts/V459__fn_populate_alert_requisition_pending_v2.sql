-- Function: fn_populate_alert_requisition_pending(integer)

DROP FUNCTION IF EXISTS fn_populate_alert_requisition_pending(integer);

CREATE OR REPLACE FUNCTION fn_populate_alert_requisition_pending(in_flag integer)
...
msg := 'Success!!! fn_populate_alert_requisition_pending - Data saved successfully' ;
...
if in_flag = 1 then
 ...
 v_last_run_date = (select now() - interval '12 month'::interval)::date;
end if;
...
delete from alert_requisition_pending where alertsummaryid not in (select alertsummaryid from alert_summary);
...
VALUES (rec_summary.rnr_summary_count, 'Requisition pending', rec_summary.geographiczoneid, 
        'REQUISITION_PENDING', rec_summary.programid,rec_summary.periodid, v_this_run_date);
end loop;

DELETE FROM alert_requisition_pending 
where COALESCE(modifieddate,v_last_run_date) >= v_last_run_date;

-- get detail pending data and stored in detail table
FOR rec_detail IN
...
and alerttypeid = 'REQUISITION_PENDING';
...
EXCEPTION
WHEN OTHERS THEN
RETURN 'Error!!! fn_populate_alert_requisition_pending.' || SQLERRM ;
END ;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION fn_populate_alert_requisition_pending(integer)
  OWNER TO openlmis;
