-- Function: fn_populate_alert_requisition_emergency(integer)

DROP FUNCTION IF EXISTS fn_populate_alert_requisition_emergency(integer);

CREATE OR REPLACE FUNCTION fn_populate_alert_requisition_emergency(in_flag integer)
...
msg := 'Sucess!!! fn_populate_alert_requisition_emergency.' ;
...
delete from alert_summary where alerttypeid = 'REQUISITION_EMERGENCY' 
and  COALESCE(modifieddate,v_last_run_date) >= v_last_run_date;

delete from alert_requisition_emergency where alertsummaryid not in (select alertsummaryid from alert_summary);
...
VALUES (rec_summary.rnr_summary_count, 'Requisition emergency', rec_summary.geographiczoneid, 
        'REQUISITION_EMERGENCY', rec_summary.programid,rec_summary.periodid, v_this_run_date);
end loop;

DELETE FROM alert_requisition_emergency 
where COALESCE(modifieddate,v_last_run_date) >= v_last_run_date;

-- get detail emergency data and stored in detail table
FOR rec_detail IN
...
and alerttypeid = 'REQUISITION_EMERGENCY';
...
EXCEPTION
WHEN OTHERS THEN
RETURN 'Error!!! fn_populate_alert_requisition_emergency. ' || SQLERRM ;
END ;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION fn_populate_alert_requisition_emergency(integer)
  OWNER TO openlmis;
