CREATE OR REPLACE VIEW vw_repair_management_not_functional AS 
 SELECT notfunctional.programid AS pid,
   ...
   FROM ( SELECT DISTINCT ON (eis.inventoryid) eis.inventoryid,
          ...
          WHERE et.iscoldchain IS TRUE AND eis.notfunctionalstatusid IS NOT NULL
