...
number_of_facilities_stocked_out_facility integer, 
number_of_facilities_stocked_out_upper integer, 
price numeric) AS
$BODY$
*/
...
-- set ggroup by
t_group_by_1 = ' group by productcode, product_short_names.name, processing_periods.startdate::date '; 
...
product_short_names.name short_name, 
...
INNER JOIN product_short_names ON product_short_names.productid = products.id
'||
...
CROSS JOIN products
join product_short_names on product_short_names.productid = products.id  
where processing_periods.id in ('|| t_periods ||') 
and product_short_names.productid in ('|| t_user_products ||') 
