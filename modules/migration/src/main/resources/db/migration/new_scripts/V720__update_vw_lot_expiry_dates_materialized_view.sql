...
DO $do$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relkind = 'm' and relname = 'vw_lot_expiry_dates_tmp') THEN
    CREATE MATERIALIZED VIEW vw_lot_expiry_dates_tmp AS
...
DROP MATERIALIZED VIEW vw_lot_expiry_dates;

ALTER MATERIALIZED VIEW vw_lot_expiry_dates_tmp RENAME TO vw_lot_expiry_dates;
