package org.openlmis.report.generator;

import org.apache.poi.xssf.usermodel.XSSFColor;

public enum StockOnHandStatus {

    REGULAR_STOCK("stock.cmm.regular.stock", "regularStock",
            new XSSFColor(new java.awt.Color(140,211,104))),

    LOW_STOCK("stock.cmm.low.stock", "lowStock",
            new XSSFColor(new java.awt.Color(250,215,77))),

    STOCK_OUT("stock.cmm.stock.out", "stockOut",
            new XSSFColor(new java.awt.Color(245,33,45))),

    OVER_STOCK("stock.cmm.over.stock", "overStock",
            new XSSFColor(new java.awt.Color(187,149,227))),

    NOT_EXIST(null, null, null);

    private String messageKey;

    private String description;

    private XSSFColor color;

    StockOnHandStatus(String key, String desc, XSSFColor color) {
        this.messageKey = key;
        this.description = desc;
        this.color = color;
    }

    public String getMessageKey() {
        return messageKey;
    }

    public String getDescription() {
        return description;
    }

    public XSSFColor getColor() {
        return color;
    }
}
