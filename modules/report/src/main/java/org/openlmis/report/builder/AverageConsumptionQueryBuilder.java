...
public class AverageConsumptionQueryBuilder {
    ...
    public static String SelectFilteredSortedPagedAverageConsumptionSql(Map params){
        ...
        SELECT("distinct  coalesce( avg(quantitydispensed),0) average, product, productcode, productcategory category, ft.name facilityType, f.name facilityName,MAX(ps.name) schedulename, MAX(pp.name) periodname,  MAX(s.name) supplyingFacility, MAX(ft.nominalmaxmonth) MaxMOS, MAX(ft.nominaleop) minMOS");
        ...
        WHERE("r.status in ('APPROVED', 'RELEASED') AND quantitydispensed > 0");
        if(filter != null){
