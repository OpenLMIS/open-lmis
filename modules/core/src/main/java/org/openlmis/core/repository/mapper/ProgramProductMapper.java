...
public interface ProgramProductMapper {
    ...
    @Select("SELECT p.id AS product_id, p.code AS productCode, #{programCode} AS programCode, " +
            "p.primaryName, p.dispensingUnit, p.dosageUnitId, p.formId, p.strength, p.dosesPerDispensingUnit, " +
    ...
    @Results(value = {
            @Result(property = "product", column = "product_id", javaType = Product.class, one = @One(select = "org.openlmis.core.repository.mapper.ProductMapper.getById")),
