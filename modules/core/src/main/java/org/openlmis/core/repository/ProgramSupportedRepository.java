...
public class ProgramSupportedRepository {
            ...
            while (newPSIterator.hasNext()) {
                ...
                if (previousProgramSupportedId.equals(newProgramSupportedId)) {
                    if (changeInProgramSupported(previousProgramSupported, newProgramSupported) ||
                            changeInReportType(previousProgramSupported, newProgramSupported)) {
    ...
    private Boolean changeInReportType(ProgramSupported ps1, ProgramSupported ps2) {
        if (ps1.isReportActive() != ps2.isReportActive()) {
            return true;
        }

        if (ps1.getReportStartDate() == null) {
            return ps2.getReportStartDate() != null;
        }

        return !ps1.getReportStartDate().equals(ps2.getReportStartDate());
    }
