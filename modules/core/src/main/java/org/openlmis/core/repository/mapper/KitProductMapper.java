...
public interface KitProductMapper {
    ...
    @Select({"SELECT * FROM kit_products_relation kp " +
            "INNER JOIN products p ON kp.productId = p.id " +
            "WHERE kp.kitId = #{kitId}"})
    @Results(value = {
    ...
    List<KitProduct> getByKitId(long kitId);


    @Select({"SELECT * FROM kit_products_relation kp " +
            "INNER JOIN products p ON kp.productId = p.id " +
            "WHERE kp.kitId = #{kitId} " +
            "AND kp.modifieddate > #{afterUpdatedTime}"})
    @Results(value = {
            @Result(property = "id", column = "id"),
            @Result(property = "quantity", column = "quantity"),
            @Result(property = "kit", column = "kitId", javaType = Kit.class,
    ...
    List<KitProduct> getLatestKitProductByKitId(@Param("kitId") long kitId, @Param("afterUpdatedTime") Date afterUpdatedTime);
}
